...
def plot_detector(data, colorize = False):
    ...
    plt.title("Detector = %s, Phase = %s, $\\phi$ = %s °\na = (%s x, %s y, %s z)\nb = (%s x, %s y, %s z)\nc = (%s x, %s y, %s z)\n rotations: %s °$\perp$ x, %s °$\perp$ y, %s °$\perp$ z"%(data["detector"].detector_type, data["crystal"]["phase"], data["detector"].tilting_angle, a_x, a_y, a_z, b_x, b_y, b_z, c_x, c_y, c_z, data["crystal"]["orientation"][0], data["crystal"]["orientation"][1], data["crystal"]["orientation"][2]))
    
    [plt.scatter(y_val, z_val, label=label) for y_val, z_val, label in zip(data["y_coordinate"], data["z_coordinate"], data["hkls"])]

    if colorize == True:
        utils.colorize(vector = list(range(len(data["hkls"]))), cmap = plt.cm.jet)
