...
def reciprocal_space(phase, rotx, roty, rotz, wavelength, E_bandwidth, smallest_number, largest_number, initial_crystal_orientation = None):
    ...
    a_rec, b_rec, c_rec = utils.cal_reciprocal_vector(a, b, c)

    hkls = utils.create_possible_reflections(phase, smallest_number, largest_number)

    #Calculating the momentum transfer vectors
    Q_hkls = [utils.calculate_Q_hkl(hkl, a_rec, b_rec, c_rec) for hkl in hkls]
...
def detector(phase, rotx, roty, rotz, detector, sample_detector_distance, tilting_angle, wavelength, E_bandwidth, smallest_number, largest_number, initial_crystal_orientation = None):
    ...
    data["crystal"]["lattice_params"] = [a,b,c]
    ...
    for w, Q_hkl in enumerate(Q_hkls):
        if utils.check_Bragg_condition(Q_hkl, wavelength, E_bandwidth) == True: 
            ...
            if utils.diffraction_in_detector(dx, dy, dz, detector.Max_Detectable_Z(), detector.Max_Detectable_Y()) == True:
    ...
    for rotx in rots_x:
        for rotz in rots_z:
            ...
            for hkl in desired_reflections_list:
                Q_hkl =  utils.calculate_Q_hkl(hkl, a_rec, b_rec, c_rec)

                if utils.check_Bragg_condition(Q_hkl, wavelength, E_bandwidth) == True: 
                    dx, dy, dz = utils.diffraction_direction(Q_hkl, wavelength, sample_detector_distance, tilting_angle)

                    if utils.diffraction_in_detector(dx, dy, dz, detector.Max_Detectable_Z(), detector.Max_Detectable_Y(), margin = margin) == True:
                        G = np.append(Q_hkl, 1)
                    else:
                        G = np.append(Q_hkl, 0)
                else:
...
def polycrystalline_sample(phase, detector, angular_step, sample_detector_distance, tilting_angle, wavelength, E_bandwidth, smallest_number, largest_number, calculate_intensities = False, margin = 0, initial_crystal_orientation = None):
    ...
    fig_size = (8*(detector.height/170)*detector.width/detector.height + 0.3, 8*(detector.height/170))
