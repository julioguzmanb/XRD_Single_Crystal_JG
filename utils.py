...
def diffraction_direction(Q_hkls, detector, wavelength):
    ...
    try:
        ...
        denominator = np.sin(np.pi/2 + tilting_angle - np.arccos(1/np.sqrt(1 + (kfxz[:, 2]/kfxz[:, 0])**2)))

        dz = (sample_detector_distance*kfxz[:, 2]/kfxz[:, 0] + beam_center[1])*(1 - np.tan(tilting_angle)*np.sin(np.arccos(1/np.sqrt(1 + (kfxz[:, 2]/kfxz[:, 0])**2)))/denominator)
    ...
    try:
        dy = (kfxy[:, 1]/kfxy[:, 0])*(sample_detector_distance - dz*np.sin(tilting_angle)) + beam_center[0]
